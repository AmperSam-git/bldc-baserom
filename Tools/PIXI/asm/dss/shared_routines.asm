...
.setup_gfx_queue                    ; no gfx loaded, proceed to load it
    tax
    lda.l exgfx_length,x            ; check if the current gfx has a valid tile length
    beq .abort_queue
    lda.w !dss_maximum_tiles        ; check if there's enough space to upload graphics
    sec
    sbc.w !dss_loaded_tiles
    cmp.l exgfx_length,x            ; if there aren't enough free tiles, abort filling up the queue
...
.fill_data
    ...
    lda.l exgfx_length,x            ; add up tile numbers
