...
;; Ice Bro
;; By Sonikku
;; Description: Walks back and forth, frequently throwing 2 iceballs in the
;; direction of the player and will attempt to catch them when they return.
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;

!GFX_FileNum = $81		;EXGFX number for bro

; Extended sprite number (from list.txt) of the iceball (iceball.asm).
!IceballNumber = $04

; The sprite tile to use for the iceball.
!IceballTile = $2D
...
+   STA !B6,x                       ; apply x speed.
    ...
    LDA !1588,x                     ; no speed if touching wall.
    AND #$03
    BEQ +
         STZ !B6,x
...
Graphics:
lda #!GFX_FileNum                  ;load boomerang bro
%FindAndQueueGFX()
bcs .gfx_loaded
...
+   TYA
    ...
    BEQ .noIceball
    PHX
    LDX #$FE                        ; iceball x position
    LDA $02
    BEQ +
        LDX #$0A                    ; iceball x position
+   TXA
    CLC : ADC $00
    STA $0300|!Base2,y

    LDA $01
    CLC : ADC #$F6                  ; iceball y position
    STA $0301|!Base2,y

    LDA.b #!IceballTile             ; iceball tilemap
    STA $0302|!Base2,y
...
assert !IceballPalette >= $08 && !IceballPalette <= $0F, "The palette of the iceball must be between 8 and F!"

IceballFlip:
    db (!IceballPalette-$08<<$01)|(!IceballPage&$01)|$80,(!IceballPalette-$08<<$01)|(!IceballPage&$01)|$C0
...
+   LDA.b #!IceballNumber+!ExtendedOffset
    STA $170B|!Base2,y              ; ext. sprite number.
    LDA #$C0                        ; ext. sprite y speed.
    STA $173D|!Base2,y
    ...
    STA $1765|!Base2,y
    TAX
    LDA Iceball_X_Speed,x           ; set up ext. sprite speed.
    STA $1747|!Base2,y
    TXA
    LDX $15E9|!Base2
...
Iceball_X_Speed:
    db $FE,$02                      ; not recommended to change.
